 ...
 * @author -MSI-
 */
public class MarkDBContext extends DBContext {
    ...
    public ArrayList<Mark> getMarkByTermAndCourse(String email, String termid, int courseid) {
        ArrayList<Mark> marks = new ArrayList<>();
        try {
            String sql = "select mc.gradeCategory as grade_category,mc.gradeItem as grade_item,mc.[weight],ms.[value],ms.comment,g.courseId,g.id as gid from mark_student ms \n"
                    ...
                    + "where s.email =? and g.termID=? and c.id=? and g.termID <5";
            PreparedStatement stm = connection.prepareStatement(sql);
...
//    }
    ...
    public ArrayList<Group> getTermOfStudentLearned(String email) {
        ArrayList<Group> groups = new ArrayList<>();
        try {
            String sql = "select distinct g.termID from [group] g join student_group sg on g.id=sg.groupid join Student s on s.id=sg.Studentid and s.email like ?"
                    + "where g.termID < 5";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setString(1, email);
            ResultSet rs = stm.executeQuery();
    ...
    public ArrayList<Group> getSelectTermToSeeMark(String email, String termId) {
        ArrayList<Group> groups = new ArrayList<>();
        try {
            String sql = "select g.id as gid,g.termID,g.courseId,g.[name],g.timeStart,g.timeEnd from [group] g \n"
                    ...
                    + "	join student s on s.id=sg.Studentid\n"
                    + "	where s.email like ? and g.termId like ?";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setString(1, email);
            stm.setString(2, termId);
            ResultSet rs = stm.executeQuery();
...
// ham ben tren chi de nhap thoi. xem thoi. ham nay de thuc hien viec insert vao database ne
    ...
    public void updateTotalForMark(String email, int gid, String total, String status, String comment) {
        try {
            String sql = "update statusMarkCourse\n"
                    + "set	total = ?, [status]  = ?,comment =  ?\n"
                    + "where student_group_id = (select sg.id from student_group sg\n"
                    + "join student s on sg.studentId = s.id\n"
                    + "where s.email = ? and sg.groupid = ?)";
            PreparedStatement stm = connection.prepareStatement(sql);
